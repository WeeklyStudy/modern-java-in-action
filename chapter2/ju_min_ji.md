...
- 다중 상속
 ...
 따라서 **인터페이스**는 일반화된 계약(Contract)을 제공하는 `기본 설계도`로 사용되며, Java와 같이 클래스의 다중 상속을 지원하지 않는 환경에서 `클래스 간의 매개체` 역할을 한다. 이러한 특성으로 인터페이스는 추상 클래스와 구별되는 특징을 가지게 된다.
...
- 또한, 객체지향 5원칙 SOLID 중 하니인 `DIP`를 지키기 위해 사용된다.
  ...
  - DIP를 위반한 코드
    ...
    ```
     ...
     - 위 코드에서는 MemberRepository의 `구현체`인 MemoryMemberRepository를 선언해서 사용하고 있기 때문에 DIP를 위반하고 있다.
  ...
  - DIP를 준수한 코드
    ...
    - 위 코드에서는 MemberService가 MemberRepository `인터페이스`에 의존하기 때문에 DIP를 준수하고 있다.

#### 💡 DIP(Dependency Inversion Principle, 의존성 역전 원칙)란?

> - DIP는 고수준 모듈은 저수준 모듈에 의존해서는 안 된다는 클래스 간의 관계를 설명하는 원칙이다. <br>
...
> - 쉽게 풀자면, 구현 클래스에 의존(혹은 참조)하지 말고 인터페이스에 의존하라는 의미이다.
