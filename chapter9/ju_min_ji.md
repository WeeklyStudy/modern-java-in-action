...
- 기존 코드에서 여러 처리 단계를 한 곳에서 중앙집권적으로 책임을 지고 있었다면, 책임을 분리하여 각 단계 별 핸들러를 생성하여 이를 연결하면 유연하게 독립적인 코드로 수정할 수 있다.
    - 특히 중첩된 여러 개의 조건문을 의무 체인 패턴으로 변환하면, 초기에는 코드 양이 증가하고 복잡해 보일 수 있지만, 나중에는 확장 및 유지보수가 더 편리하고 유연한 구조가 된다.
...
- 장점:
    - 낮은 결합도
        ...
        ![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/60f861e8-82d5-4c38-96bf-76ce89b191d8/Untitled.png)
...
- 목적(생성, 구조, 행동)과 범위(클래스에 적용, 객체에 적용)에 따라 분류할 수 있다.
    ...
    ![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/b4505240-1971-4549-945e-11bb90aec2ff/Untitled.png)
...
- 디자인 패턴들은 아래와 같은 관계도를 가진다.
    ...
    ![Untitled](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/63800daa-e557-48b7-b7e4-cbe4e73a4957/Untitled.png)
    
    - Composite 패턴은 Iterator 패턴과 Visitor 패턴과 함께 사용해야 하는 경우가 대부분이고, Prototype 패턴은 Factory 패턴의 대안이 되기도 한다.
- 또한, 패턴들은 의도는 다르지만 매우 유사한 구조를 가지기도 한다.
- 따라서, 각각의 디자인 패턴들을 비교하여 언제 어떻게 적용할 것인가에 대한 고민을 거쳐야 한다.

### References

- https://soopiri.tistory.com/6
